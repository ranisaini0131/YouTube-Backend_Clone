...
const publishAVideo = asyncHandler(async (req, res) => {
    ...
    if ([title, description, videoFile, thumbnail].some((field) => field?.trim() === "")) {
        throw new ApiError(400, "All field are required");
    }


    // get local path of video and thumbnail
    ...
    // Save the video in database and get
    const video = await Video.create({
        title,
        description,
        duration: cloudinaryVideoFile.duration,
        videoFile: {
            url: cloudinaryVideoFile.url,
            public_id: cloudinaryVideoFile.public_id
        },
        thumbnail: {
...
const updateVideo = asyncHandler(async (req, res) => {
    ...
    const thumbnailLocalPath = req.files?.thumbnail[0].path
    console.log(thumbnailLocalPath, "237")
